...
BST_X_DATA_DIR to the collated npy_[3d_][seq{N}_]{split}_{collation_id} directory
(the 3d_/seq{N}_ prefixes appear only for non-default configs) and run pytest:

    BST_X_DATA_DIR=/path/to/npy_v2_taxon_pinned_w_preds \
...
@pytest.mark.skipif(
    not BST_X_DATA_DIR or not Path(BST_X_DATA_DIR).exists(),
    reason=(
        "BST_X_DATA_DIR not set or does not exist. "
        "Set BST_X_DATA_DIR to an npy_[3d_][seq{N}_]{split}_{collation_id} directory "
        "on HPC and re-run."
    ),
)
def test_pipeline_dataloader_to_model_forward_pass():
    ...
    # only for non-default configs) and the steps below will validate the full
    # downstream pipeline.
    # -------------------------------------------------------------------------
    ...
    test_dir = Path(BST_X_DATA_DIR) / "test"
    ...
    dataset = Dataset_npy_collated(Path(BST_X_DATA_DIR), "test", pose_style=pose_style)
    assert len(dataset) > 0, (
        "Dataset is empty — check BST_X_DATA_DIR points to the correct "
        "npy_[3d_][seq{N}_]{split}_{collation_id} directory and that the test/ split exists."
    )
