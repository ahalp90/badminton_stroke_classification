...
BST_DATA_DIR to the collated npy_[3d_][seq{N}_]{split}_{collation_id} directory
(the 3d_/seq{N}_ prefixes appear only for non-default configs) and run pytest:

    BST_DATA_DIR=/path/to/npy_v2_taxon_pinned_w_preds \
...
@pytest.mark.skipif(
    ...
    reason=(
        ...
        "Set BST_DATA_DIR to an npy_[3d_][seq{N}_]{split}_{collation_id} directory "
        "on HPC and re-run."
    ),
)
