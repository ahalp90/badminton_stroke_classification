...
- `bst_refactor_deprecated/` — was `src/bst_x/deprecated/`. Author's pre-phase-1 scripts (`gen_my_dataset.py`, `get_each_class_total.py`, etc.), the pre-flatten snapshot of `pipeline/` and `stroke_classification/` (`before_flattening_asset_dirs/`), and phase-0 historical documentation (`outdated_*.md`, `historical_*.md`).
- `shuttleset_deprecated/` — was `src/bst_x/ShuttleSet/deprecated/`. The original BST author's pre-refactor ShuttleSet helpers.
- `main_on_shuttleset_tmp/` — was `src/bst_x/stroke_classification/main_on_shuttleset/tmp/`. Phase-0 smoke-test scripts (`test_dataloader.py`, `test_fwd.py`, `test_train_step.py`).
