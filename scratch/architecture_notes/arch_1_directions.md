...
Motivation: a per-class shuttle-zeroing audit run during the capacity-bump exploration revealed that smash and wrist_smash had the worst shuttle-frame-zeroing rates of any class by a magnitude (whole-clip mean 13.7% smash, 8.5% ws, 1.6x ratio). Across the rest of the classes the rate was small. The collation step (`prepare_train_on_shuttleset.py:866-867`) was wiping `shuttle[failed,:] = 0` whenever any pose slot failed for a frame — an asymmetry that didn't exist in the other direction (pose flowed through unchanged on shuttle-fail frames). Roughly 14k frames (0.84% of extract) carried this asymmetric wipe, concentrated on the bottleneck pair.
