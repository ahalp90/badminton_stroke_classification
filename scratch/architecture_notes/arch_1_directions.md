...
- **Focal loss ablation**: **basic focal skipped per project decision**. Vanilla / manually-alpha focal would hit the same central-tendency ceiling that static class weighting just hit (same lever, per-sample-gated). Jumping straight to **CDB-F1** (class-F1-driven adaptive focal) as the next loss-side cell — its bimodal-seed-escape mechanism is the right fit for the failure mode the smoke test revealed. Design doc verified against the ACCV 2020 paper at `scratch/architecture_notes/class_f1_focal_design.md`; pair-aware Seesaw-F1 alternative held as second arm at `scratch/architecture_notes/seesaw_f1_focal_design.md`.
...
## Next: CDB-F1 directly (basic focal skipped)

The earlier "Next: class weighting then focal" plan had three loss-side cells gated on each other. The first cell (pair-balanced class weighting) landed and revealed a bimodal failure mode that revoked the basic-focal step's gating logic. Restated runbook below.

### Why skip basic focal

Vanilla focal `(1-p_t)^γ * -log(p_t)` and manually-alpha focal `α[c] * (1-p_t)^γ * -log(p_t)` are the same lever as class-weighted CE, just gated by per-sample confidence. The class-weighting smoke test (`run_20260501_110525`) showed the static-reweighting lever **doesn't move the central tendency** past LS=0.15 — pair-balanced 2.0/2.0 added +0.005 mean wrist_smash, inside seed variance. Adding `(1-p_t)^γ` on top concentrates that same gradient on hard samples, but the magnitude bound on the per-class lever is unchanged. Vanilla focal would hit the same ceiling on the mean.

What the smoke test *did* reveal is a bimodal seed distribution: one seed (S2) found a wrist_smash basin worth 0.518 — a new project-wide ceiling — that the other four seeds didn't access. The right next escalation isn't "more aggressive static reweighting" (focal) but "adaptive reweighting that escalates harder when a class stays low across epochs". That's CDB-F1 specifically.

### Cell 3 (now Cell 1 of the adaptive arc): CDB-F1

Per-class alpha = `(1 - F1_c)^τ` where F1_c is an EMA-smoothed running per-class F1 on the train set, computed end-of-epoch from a per-class TP/FP/FN counter that gets accumulated during the existing forward pass (no second forward pass needed). Optionally compose with focal `(1-p_t)^γ` per-sample focusing. Defaults: `τ=1.0`, `γ=1.0`, EMA momentum 0.9, 5-epoch warm-up running uniform CE.

Full design at `scratch/architecture_notes/class_f1_focal_design.md` — equations verified verbatim against the ACCV 2020 paper; reported lifts on EGTEA (19-class video action, +2.17 pp minority recall vs unweighted CE) and CIFAR-100-LT IF=100 (+4.36 pp top-1 vs unweighted CE) verified against tables 3, 4, 7 of the paper directly. **Caveat captured in the design doc**: the published headline numbers were achieved with dynamic τ; our default fixed τ=1.0 trails by ~0.9 pp on the same benchmark, which is still a clear lift over uniform CE. Dynamic-τ escalation is a follow-up cell if fixed τ partially lifts.

Implementation surface: ~120-line new module at `src/bst_refactor/stroke_classification/main_on_shuttleset/loss/adaptive_focal.py` + ~25 lines across 6 edits in `bst_train.py` (new `Hyp.adaptive_focal` field, mutex-checked branch in the loss build, TP/FP/FN accumulator in `train_one_epoch`, end-of-epoch alpha updater, per-class TensorBoard logging, console top-3/bot-3 alpha printout). Manifest auto-serialises the new Hyp field via `_asdict()`.

`label_smoothing=0` forced in the CDB-F1 branch (LS softens targets so confident-correct samples have `p_t < 1.0`, contaminating focal's hardness estimate). Step away from the current LS=0.15 active for this cell; comparison baseline is then both LS=0.1 (the canonical baseline) and LS=0.15 + class-weights (the run_20260501_110525 cell that produced the S2 ceiling).

### LR schedule note

No schedule adjustment expected for the first CDB-F1 cell:

- Optimiser is **AdamW** (`bst_train.py:343`), which is roughly scale-invariant per-parameter due to the second-moment estimate. So focal's `(1-p_t)^γ`-induced gradient-magnitude shrink (~30% on average at γ=1) doesn't translate 1:1 to "need 30% more LR" the way it would for plain SGD.
- Per-class alpha renormalised to mean=1.0 inside `update_alpha`, so the *average* effective LR across classes stays comparable to vanilla CE; only the *shape* of which class gets which fraction shifts.
- The 5-epoch alpha warm-up runs uniform CE, so the LR cosine warmup-then-decay sees a familiar loss surface during its first ramp.
- Standard `lr=5e-4`, `warm_up_step=100`, `num_cycles=0.5`, `n_epochs=80` carries through unchanged for the first cell.

If-then rules to apply if symptoms appear:

- **Train loss decays much slower than the LS=0.15 baseline** → bump LR to 7.5e-4 or 1e-3. Symptom: train loss at epoch 20 is more than ~0.05 above the LS=0.15 cell's epoch-20 train loss.
- **Alpha update doesn't move mean wrist_smash by epoch 30** → consider longer training (extend `n_epochs` to 120) or shorten alpha warmup (drop `warm_up_epochs` from 5 to 3). Symptom: alpha trajectory in TensorBoard shows the wrist_smash α has converged (EMA flatlined) but mean wrist_smash F1 is still in the 0.40-0.42 band.
- **Bimodal seed distribution persists** (some seeds find wrist_smash 0.5+, others stay at 0.4) → adaptive isn't enough; escalate to dynamic τ (CDB eq. 3-4, ~20 LOC addition to `update_alpha`) to amplify the bias signal on persistently-bad seeds.

### Cell 2 (held): Seesaw-F1

If CDB-F1 lifts wrist_smash mean but at smash's expense (the "stole recall from the pair partner" failure mode), the natural escalation is the pair-aware Seesaw-F1 design at `scratch/architecture_notes/seesaw_f1_focal_design.md`. Design verified against the CVPR 2021 paper (sections 3, 4.1-4.5, tables 1-11). Larger code surface (~180-line new module + custom logsumexp forward) and not invoked unless CDB-F1's smash-drop diagnostic flags it.

### Decision tree

| CDB-F1 result | Next |
|---------------|------|
| Mean wrist_smash ≥ 0.42 AND macro within 0.5 pp of LS=0.15 baseline | Run dynamic-τ CDB-F1 (eq. 3-4 escalation) as a refinement cell |
| Mean wrist_smash ≥ 0.42 BUT smash drops ≥2 pp | Pivot to Seesaw-F1 (pair-aware variant) |
| Mean wrist_smash flat or worse | Loss-side knobs comprehensively exhausted; pivot to augmentation (horizontal flip + tightened random shift) or jump straight to X3D-S |

X3D-S remains the long-term solution regardless — adds racket-pixel information pose-only can't see. Augmentation is the cheaper intermediate step in the loss-side-exhausted branch.
